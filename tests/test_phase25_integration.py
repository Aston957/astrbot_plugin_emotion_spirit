...
def test_e2e_segment_change_changes_tone():
    ...
    # 验证段变化
    assert seg_a != seg_b
    # 验证 tone 不同 (至少 warmth 应该有差异)
    assert tone_a.get("warmth", 0) != tone_b.get("warmth", 0)
    # 验证 delta 累加 (warmth 上升)
    assert delta_b.get("warmth", 0) > delta_a.get("warmth", 0)


def test_e2e_apply_to_layers_creates_effective():
    """apply_to_layers: base + delta → effective_personality。"""
    rp = RelationshipPersonality()
    rp.set_delta("alice", "warmth", 0.15)
    rp.set_delta("alice", "intimacy_pull", 0.1)

    # base (deep + surface 格式)
    base = {
        "deep": {"warmth": 0.5, "intimacy_pull": 0.4, "autonomy": 0.7},
        "surface": {"warmth": 0.6, "intimacy_pull": 0.5, "autonomy": 0.7},
    }
    effective = rp.apply_to_layers(base, "alice")

    # deep.warmth: 0.5 + 0.15 = 0.65
    assert effective["deep"]["warmth"] == 0.65
    ...
    # surface 同样
    assert effective["surface"]["warmth"] == 0.75


def test_e2e_apply_to_layers_clamps_to_0_1():
    """apply_to_layers 后的值应 clamp 到 [0, 1]。"""
    rp = RelationshipPersonality()
    rp.set_delta("alice", "warmth", 0.3)  # 累加上限
    base = {"deep": {"warmth": 0.9}}
    effective = rp.apply_to_layers(base, "alice")
    # 0.9 + 0.3 = 1.2 → clamp 1.0
    assert effective["deep"]["warmth"] == 1.0
...
def test_e2e_per_user_delta_isolation_in_effective():
    ...
    base = {"deep": {"warmth": 0.5}}
    eff_a = rp.apply_to_layers(base, "alice")
    eff_b = rp.apply_to_layers(base, "bob")
    # alice: 0.5 + 0.2 = 0.7
    assert eff_a["deep"]["warmth"] == 0.7
    # bob: 0.5 - 0.1 = 0.4
    assert eff_b["deep"]["warmth"] == 0.4


def test_e2e_apply_tone_accumulates_not_replaces():
    """apply_tone 累加: 多次调用不应覆盖既有 delta。"""
    rp = RelationshipPersonality()
    rp.apply_tone("alice", {"warmth": 0.05, "expression_drive": 0.03})
    rp.apply_tone("alice", {"warmth": 0.05})
    delta = rp.get_delta("alice")
    # warmth 应累加为 0.10
    assert abs(delta["warmth"] - 0.10) < 1e-9
...
def test_e2e_full_intimacy_to_effective_flow():
    ...
    # 5. 应用到 base
    base = {"deep": {"warmth": 0.5, "intimacy_pull": 0.4}}
    effective = rp.apply_to_layers(base, "alice")

    # 6. 验证: 高亲密段应有正向 warmth delta
    delta_warmth = rp.get_single_delta("alice", "warmth")
    assert delta_warmth >= 0  # 至少不降低
    # effective warmth 应 >= base warmth
    assert effective["deep"]["warmth"] >= base["deep"]["warmth"]


def test_e2e_stranger_vs_inner_circle_opposite_tone():
    """stranger 和 inner_circle 的 warmth tone 方向相反。"""
    tracker = IntimacyTracker()
    # alice: 无交互 → stranger
    ...
    # stranger warmth 应 < 0
    assert tone_stranger["warmth"] < 0
    # inner_circle warmth 应 > 0
    assert tone_inner["warmth"] > 0
    # 反向
    assert tone_stranger["warmth"] * tone_inner["warmth"] < 0
...
def test_e2e_serialization_after_apply_tone():
    ...
    rp1.apply_tone("alice", {"warmth": 0.1, "intimacy_pull": 0.2})
    rp1.apply_tone("bob", {"warmth": -0.05})

    data = rp1.to_dict()
    rp2 = RelationshipPersonality.from_dict(data)
    # alice 保留
    assert abs(rp2.get_single_delta("alice", "warmth") - 0.1) < 1e-9
    assert abs(rp2.get_single_delta("alice", "intimacy_pull") - 0.2) < 1e-9
    # bob 保留
    assert abs(rp2.get_single_delta("bob", "warmth") - (-0.05)) < 1e-9
