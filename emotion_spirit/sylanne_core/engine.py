...
class SylanneEngine:
    ...
    _shared_instances: dict[str, "SylanneEngine"] = {}
    ...
    async def shared(
        ...
        """获取或创建共享引擎实例（同一 data_dir 只有一个实例）。

        Args:
            data_dir: 持久化目录（作为共享 key）。
            llm: 异步 LLM 函数。
            embedding: 可选 embedding 函数。
            config: 引擎配置。

        Returns:
            共享的 SylanneEngine 实例。

        Raises:
            SharedEngineConflictError: 同一 data_dir 第二次传入不同 config。
        """
        key = str(Path(data_dir).resolve())
        if key in cls._shared_instances:
            existing = cls._shared_instances[key]
            if config is not None and config != existing._config:
                raise SharedEngineConflictError(
                    f"Shared engine at {key} already exists with different config"
                )
            if llm is not existing._llm:
                logger.warning(
                    "SylanneEngine.shared: data_dir=%s 传入不同 llm, 复用原实例",
                    key,
                )
            return existing

        engine = cls(data_dir=data_dir, llm=llm, embedding=embedding, config=config)
        await engine.start()
        cls._shared_instances[key] = engine
        logger.info("SylanneEngine.shared: 创建共享实例 data_dir=%s", key)
        return engine
    ...
    async def release_shared(cls, data_dir: str | Path) -> None:
        """释放共享实例（flush 落盘 + 关闭）。"""
        key = str(Path(data_dir).resolve())
        engine = cls._shared_instances.pop(key, None)
        if engine is not None:
            await engine.shutdown()
            logger.info("SylanneEngine.shared: 释放实例 data_dir=%s", key)

    @classmethod
    def list_shared(cls) -> list[dict[str, str]]:
        """列出所有共享实例。"""
        return [
            {"data_dir": k, "status": e.status}
            for k, e in cls._shared_instances.items()
        ]
    ...
    def is_shared(cls, data_dir: str | Path) -> bool:
        """检查 data_dir 是否有共享实例。"""
        key = str(Path(data_dir).resolve())
        return key in cls._shared_instances
